...
// @Controller + @RequestMapping 을 합친 것이 WebServlet이다.
// Spring에서는 Class 앞에 Controller를 붙이고, Method 앞에 RequestMapping으로 연결했었다.
// servlet에서는 클래스 단위로 mapping이 된다.
// 클래스를 많이 만들어야한다는 단점이 있다.
// extend HttpServlet을 무조건해야되며, 
// public void service(HttpServletRequest request, HttpServletResponse response) throws IOException 이것도 항상 적어야한다.
// 톰캣에 lib를 import해서 해결
